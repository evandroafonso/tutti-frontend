  ...
  <div class="bg-white">
    <header class="absolute inset-x-0 top-0 z-50">
      <nav class="flex items-center justify-between p-6 lg:px-8" aria-label="Global">
        <div class="flex lg:flex-1">
          <a href="#" class="-m-1.5 p-1.5">
            ...
            <img class="w-auto h-14" src="/music.svg" alt="" loading="lazy" />
      ...
      <Dialog class="lg:hidden" @close="mobileMenuOpen = false" :open="mobileMenuOpen">
        <div class="fixed inset-0 z-50 bg-black bg-opacity-25" aria-hidden="true"></div>
          ...
          <div class="flow-root mt-6">
            <div class="-my-6 divide-y divide-gray-500/10">
              <div class="py-6 space-y-2">
                <a v-for="item in navigation" :key="item.name" :href="item.href" class="block px-3 py-2 -mx-3 text-base font-semibold leading-7 text-gray-900 rounded-lg hover:bg-gray-400/10">{{ item.name }}</a>
              </div>
            </div>
          </div>
        </DialogPanel>
    ...
    <div class="relative px-6 isolate pt-14 lg:px-8">
      <section class="flex items-center justify-center h-screen snap-start">
        <div class="text-center">
          <h1 class="text-5xl font-bold tracking-tight text-gray-800 sm:text-6xl">
          ...
          <div class="flex items-center justify-center mt-10 gap-x-6">
            <a href="#" class="rounded-md bg-green-600 px-3.5 py-2.5 text-sm font-semibold text-white shadow-xs hover:bg-green-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-green-600">Get started</a>
          </div>
        </div>
      </section>
      ...
      <section class="flex items-center justify-center snap-start">
        <div class="text-center">
          <h2 class="font-semibold text-green-600 text-base/7">
            Planos e Preços
          </h2>
      ...
      <div class="grid items-center max-w-lg grid-cols-1 mx-auto mt-16 gap-y-6 sm:mt-20 sm:gap-y-0 lg:max-w-4xl lg:grid-cols-2">
        <div v-for="(tier, tierIdx) in tiers" :key="tier.id" :class="[
          ...
          'rounded-3xl p-8 ring-2 ring-gray-900/20 sm:p-10',
        ]">
          <!-- tipos dos planos: mensal ou anual -->
          <h3 :id="tier.id" :class="[
            tier.featured ? 'text-white' : 'text-green-900',
          ...
          <p class="flex items-baseline mt-4 gap-x-2">
            <span :class="[
              tier.featured ? 'text-gray-800' : 'text-gray-700',
          ...
          <!-- botão de ação -->
          <!-- Botão de ação: Dentro do template do plano -->
          <a :href="tier.href" :aria-describedby="tier.id" @mouseover="animateImage" @mouseleave="resetImageAnimation" :class="[
            tier.featured
              ? 'bg-green-500 text-white shadow-xs hover:bg-green-400 focus-visible:outline-green-500'
              : 'bg-green-200 text-gray-600 hover:text-white shadow-xs hover:bg-green-300 focus-visible:outline-green-500',
      ...
      <div class="w-full max-w-screen-xl p-4 py-6 mx-auto lg:py-8">
        <div class="md:flex md:justify-between">
          <div class="mb-6 md:mb-0">
            <a href="#" class="flex items-center">
              <img src="/music.svg" class="h-8 me-3" alt="Tutti Logo" />
              <span class="self-center text-2xl font-semibold whitespace-nowrap dark:text-white">Tutti</span>
            </a>
          </div>
          <div class="grid grid-cols-2 gap-8 sm:gap-6 sm:grid-cols-3">
            <div>
              ...
              <ul class="font-medium text-gray-500 dark:text-gray-400">
                <li class="mb-4">
                  <a href="https://flowbite.com/" class="hover:underline">Tutti</a>
                </li>
                <li>
                  <a href="https://tailwindcss.com/" class="hover:underline">Alterar</a>
                </li>
              </ul>
            </div>
            <div>
        ...
        <div class="sm:flex sm:items-center sm:justify-between">
          <span class="text-sm text-gray-500 sm:text-center dark:text-gray-400">© 2025
            <a href="https://flowbite.com/" class="hover:underline">Tutti - Musicalização Infantil</a>. All Rights Reserved.
          </span>
...
const guitarImage = ref(null);

function animateImage() {
  // Adiciona a classe para a animação
  if (guitarImage.value) {
    guitarImage.value.classList.add("animate-bounce");
  }
}

function resetImageAnimation() {
  // Remove a classe assim que o mouse sair do botão
  if (guitarImage.value) {
    guitarImage.value.classList.remove("animate-bounce");
  }
}
